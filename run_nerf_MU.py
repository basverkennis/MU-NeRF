...
from run_nerf_helpers_MU import * # added
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False):
    ...
    rgb3  = torch.sigmoid(raw[...,:3]) # added
    rgb6  = torch.sigmoid(raw[...,3:6]) # added
    rgb9  = torch.sigmoid(raw[...,6:9]) # added
    alpha = raw[...,9] # added
    uncert3 = raw[...,10] # added
    uncert6 = raw[...,11] # added
    uncert9 = raw[...,12] # added

    noise = 0.
    if raw_noise_std > 0.:
        noise = torch.randn(raw[...,9].shape) * raw_noise_std # added
        # noise = torch.randn(raw[...,3].shape) * raw_noise_std

        # Overwrite randomly sampled data if pytest
        if pytest:
            np.random.seed(0)
            noise = np.random.rand(*list(raw[...,9].shape)) * raw_noise_std # added
            # noise = np.random.rand(*list(raw[...,3].shape)) * raw_noise_std 
            noise = torch.Tensor(noise)

    alpha = raw2alpha(raw[...,9] + noise, dists)  # [N_rays, N_samples] # added
    # alpha = raw2alpha(raw[...,3] + noise, dists)  # [N_rays, N_samples] 
    weights = alpha * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)), 1.-alpha + 1e-10], -1), -1)[:, :-1]

    rgb3_map = torch.sum(weights[..., None] * rgb3, -2) # added
    rgb6_map = torch.sum(weights[..., None] * rgb6, -2) # added
    rgb_map = torch.sum(weights[...,None] * rgb9, -2)  # [N_rays, 3] # added
    # rgb_map = torch.sum(weights[...,None] * rgb, -2)  # [N_rays, 3]
    uncert3_map = torch.sum(weights * weights * uncert3, -1) # added
    uncert6_map = torch.sum(weights * weights * uncert6, -1) # added
    uncert_map = torch.sum(weights * weights * uncert9, -1) # added
    ...
    return rgb_map, disp_map, acc_map, weights, depth_map, uncert_map, \
       F.relu(raw[...,9] + noise).mean(-1), \
       rgb3_map, rgb6_map, rgb_map, uncert3_map, uncert6_map, uncert_map # added
...
def render_rays(ray_batch,
    ...
    Returns:
      ...
      raw: [num_rays, num_samples, 12]. Raw predictions from model. # added
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, uncert_map, alpha_map, rgb3_map, rgb6_map, rgb9, uncert3_map, uncert6_map, uncert9 = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest) # added
    ...
    if N_importance > 0:
        ...
        rgb_map, disp_map, acc_map, weights, depth_map, uncert_map, alpha_map, rgb3_map, rgb6_map, rgb9_map, uncert3_map, uncert6_map, uncert9_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest) # added
        # rgb_map, disp_map, acc_map, weights, depth_map, uncert_map, alpha_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
    ...
    # Multi-scale predictions added to extras
    'rgb3': rgb3_map,
    'rgb6': rgb6_map,
    'rgb9': rgb9_map,
    'uncert3': uncert3_map,
...
def config_parser():
    ...
    # parser.add_argument("--lambda_kl",   type=float, default=0.01) # Strength for regularization KL divergence # added
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            imageio.mimwrite(os.path.join(testsavedir, 'video_uncert.mp4'), to8b(uncerts), fps=30, quality=8) # added
            # or 
            # uncerts_norm = [u / np.max(u) for u in uncerts]
            # imageio.mimwrite(os.path.join(testsavedir, 'video_uncert.mp4'), to8b(uncerts_norm), fps=30, quality=8) # added
            return
    ...
    for i in trange(start, N_iters):
        ...
        optimizer.zero_grad()
        if uncert.min() > 0:
            mus = [extras['rgb3'], extras['rgb6'], extras['rgb9']] # added
            uncert_list = [extras['uncert3'], extras['uncert6'], extras['uncert9']] # added
            # weights = torch.tensor([0.5, 1.0, 2.0], device=target_s.device) # added
            # weights = torch.tensor([0.1, 1.0, 5.0], device=target_s.device) # added
            img_loss = combo_loss_full(target_s, mus, uncert_list, alpha, weights=None, lambda_kl=0.1, w=args.w) # added
...
def train_all_scenes():
    ...
    for scene in llff_scenes:
        ...
        args.expname = f'MU-1mean-noKL_llff_{scene}'
        train()
